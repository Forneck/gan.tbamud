...
types = ['.wld', '.obj', '.mob', '.qst', '.shp', '.trg', '.zon']
...
for tipo in types:
    ...
    for nome_arquivo in pathlib.Path(pasta).glob(tipo[1:] + '/*' + tipo):
        ...
        with open(nome_arquivo, 'r') as arquivo:
            texto = arquivo.read()      
            texto_codificado = encoder(texto, palavra_para_numero)
    ...
    torch.save(dados_codificados, os.path.expanduser(f'~/mud/gan/{tipo[1:]}-full.pt'))

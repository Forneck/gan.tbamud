...
criterio_cnn = torch.nn.BCELoss() #pode ser a BCELoss, mas a MSE penaliza as maiores diferenças entre a saida e os rótulos.
    ...
    for tipo in types:
        ...
        for (textos, rotulos), textos_falsos in zip(train_loaders[tipo], loader_gerador):
            ...
            if args.verbose == 'cnn' or args.verbose == 'on':
                print(f'Saida da cnn com textos reais {saida_real} e falsos {saida_falso}')
            ...
            if args.verbose == 'on' or args.verbose == 'cnn':
                print(f'Saida da cnn apos otimização: {saida_falso}')
            if args.verbose == 'on':
